...
class Bitcrush(GraphNode):
    ...
    bit_depth: int | GraphNode | tuple[GraphNode, str] = Field(
        default=8,
        description="The bit depth to reduce the audio to. Lower values create more distortion.",
    )
    sample_rate_reduction: int | GraphNode | tuple[GraphNode, str] = Field(
        default=1,
    ...
    def get_node_type(cls):
        return "lib.pedalboard.Bitcrush"
...
class Compress(GraphNode):
    ...
    ratio: float | GraphNode | tuple[GraphNode, str] = Field(
        default=4.0,
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.Compress"
...
class Delay(GraphNode):
    ...
    delay_seconds: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.5, description="Delay time in seconds."
    )
    feedback: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.3, description="Amount of delayed signal fed back into the effect."
    )
    mix: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.5,
...
class Distortion(GraphNode):
    ...
    drive_db: float | GraphNode | tuple[GraphNode, str] = Field(
        default=25.0, description="Amount of distortion to apply in decibels."
    )

    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.Distortion"
...
class Gain(GraphNode):
    ...
    gain_db: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.0,
...
class HighPassFilter(GraphNode):
    ...
    cutoff_frequency_hz: float | GraphNode | tuple[GraphNode, str] = Field(
        default=80.0, description="The cutoff frequency of the high-pass filter in Hz."
    )

    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.HighPassFilter"
...
class HighShelfFilter(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.HighShelfFilter"
...
class Limiter(GraphNode):
    ...
    threshold_db: float | GraphNode | tuple[GraphNode, str] = Field(
        default=-2.0, description="Threshold in dB above which the limiter is applied."
    )
    release_ms: float | GraphNode | tuple[GraphNode, str] = Field(
        default=250.0, description="Release time in milliseconds."
    )

    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.Limiter"
...
class LowPassFilter(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.LowPassFilter"
...
class LowShelfFilter(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.LowShelfFilter"
...
class NoiseGate(GraphNode):
    ...
    attack_ms: float | GraphNode | tuple[GraphNode, str] = Field(
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.NoiseGate"
...
class PeakFilter(GraphNode):
    ...
    q_factor: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1.0,
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.PeakFilter"
...
class Phaser(GraphNode):
    ...
    rate_hz: float | GraphNode | tuple[GraphNode, str] = Field(
        default=1.0, description="Rate of the phaser effect in Hz."
    )
    depth: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.5, description="Depth of the phaser effect."
    )
    centre_frequency_hz: float | GraphNode | tuple[GraphNode, str] = Field(
...
class PitchShift(GraphNode):
    ...
    semitones: float | GraphNode | tuple[GraphNode, str] = Field(
        default=0.0,
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.PitchShift"
...
class TimeStretch(GraphNode):
    ...
    @classmethod
    def get_node_type(cls):
        return "lib.pedalboard.TimeStretch"
